...
function isBorder(imageData, x, y) {
	const center = getPixelData(imageData, x, y)[3] < 128;;
	const left = getPixelData(imageData, x-1, y)[3] < 128;
	const right = getPixelData(imageData, x+1, y)[3] < 128;
	const top = getPixelData(imageData, x, y-1)[3] < 128;
	const down = getPixelData(imageData, x, y+1)[3] < 128;
...
function checkDist(cell, border, closestPixel) {
	const { x, y } = cell;
	const [ bx, by ] = border;
	const dx = x - bx;
	const dy = y - by;
	...
	if (dist < cell.dist) {
		cell.closest = border;
		cell.dist = dist;
		cell.closestPixel = closestPixel;
	}
}
...
function propagate(cell, cells, collector, width, height) {
	...
	neighbors.forEach((neighbor, index) => {
		...
		if (neighbor && (cell.flag & f) === 0) {
			cell.flag |= f;
			checkDist(neighbor, cell.closest, cell.closestPixel);
...
function getSDFCells(imageData) {
	...
	for (let y = 0; y < height; y++) {
		for (let x = 0; x < width; x++) {
			...
			const inside = pixel[3] >= 128;
			const border = inside && (x === 0 || y === 0 || x === width-1 || y === height-1 ? false : isBorder(imageData, x, y));
...
function createSDF(fromImageData, toImageData) {
	...
	for (let py = 0; py < toImageData.height; py++) {
		for (let px = 0; px < toImageData.width; px++) {
			const { dist, inside, closestPixel } = cells[Math.floor((py * ymul + ymul/2) * fromImageData.width + px * xmul + xmul/2)];
