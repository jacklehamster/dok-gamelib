...
function getSDFCells(ctx, cellSize) {
	...
	const borders = [];
	let leftPixel = cellSize-1, topPixel = cellSize-1, rightPixel = 0, bottomPixel = 0;
	for (let y = 0; y < cellSize; y++) {
		for (let x = 0; x < cellSize; x++) {
			const pixel = getPixelData(data, x, y, cellSize);
			const border = x === 0 || y === 0 || x === cellSize-1 || y === cellSize-1 ? false : isBorder(data, x, y, cellSize);
			const inside = pixel[0] >= 128;
			const cell = cells[y * cellSize + x] = {
				x,
				y,
				border,
				inside,
...
function createFontSheet({name, characters, cellSize, fontSize, finalCellSize}) {
	...
	for (let i = 0; i < characters.length; i++) {
		...
		const { cells, width, height } = getSDFCells(stampCtx, cellSize);
