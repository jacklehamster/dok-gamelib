...
function fitImages(imageInfo, sheet, rectX, rectY, rectWidth, rectHeight,
	...
	for (let i = 0; i < keys.length; i++) {
		...
		const { fitted, width, height, image, scenes, font } = imgData;
		if (!fitted) {
			if (width <= rectWidth && height <= rectHeight) {
				imageAdded = true;
				imgData.fitted = true;
				sheet.push({ image, key, font, scenes, x: rectX, y: rectY, width: image.width, height: image.height });
				count++;
...
function arrangeImages(images, sheetWidth, sheetHeight) {
	...
	images.forEach(image => {
		...
		if (rescale) {
			console.log(`Rescaling ${image.src}.`);
	...
	const groups = [
		{
			sdf: false, 
			keys: keys.filter(k => !imageInfo[k].rescale && !imageInfo[k].font),
		},
		{
			sdf: true, 
			keys: keys.filter(k => imageInfo[k].rescale || imageInfo[k].font),
...
function processDirectories(dirs, width, height) {
	...
	return loadImagesFromPath(dirs).then(images => {
		...
		sheets.forEach(({sheet}, index) => {
			...
			const ctx = canvas.getContext('2d');
			sheet.forEach(({image, scenes, x, y, width, height, key, font}) => {
				if (image) {
