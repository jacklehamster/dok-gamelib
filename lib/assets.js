...
function loadImagesFromPath(dir) {
	...
	return new Promise((resolve, reject) => {
		template.getFolderAsData(dir).then(items => {
			const images = items.filter(filename => {
...
function produceSpritesheets(assetDirectory, width, height) {
	return processDirectory(assetDirectory, width, height).then(({spritesheets, sprites, size, freeCells}) => {
		return new Promise((resolve, reject) => {
	...
	return new Promise((resolve, reject) => {
		template.getFolderAsData(dir).then(items => {
			...
			Promise.all(images.map(getSha)).then(shaData => {
				shaData.sort((a,b) => a.key.localeCompare(b.key));
				resolve(shaData);
    		});
		});
	});
