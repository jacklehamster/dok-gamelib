...
function copySource(webDir, dirname) {
	...
	const jsonCompactFilename = `${webDir}/generated/js/source/lib/json-compact.js`;
	return fs.promises.mkdir(path.dirname(jsonCompactFilename), { recursive: true})
		.then(fs.promises.copyFile(`generated/lib/json-compact.js`, jsonCompactFilename))
		.then(Promise.all(files.filter(file => file.startsWith("src/")).map(file => file.split("src/")[1]).map(file => {
			return fs.promises.mkdir(path.dirname(`${webDir}/generated/js/source/${file}`), { recursive: true})
				.then(fs.promises.copyFile(`src/${file}`, `${webDir}/generated/js/source/${file}`))
				.then(() => `${webDir}/generated/js/source/${file}`)
		})))
		.then(() => {
