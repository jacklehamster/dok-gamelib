...
class SpriteInstance extends AnimatedSpriteInstance {
	...
	getEvaluated(game, definition) {
		...
		const { src, animation, scale, pos, motion: { mov, gravity, time }, hotspot, corners, rotation: { angle, center }, blackhole, refresh } = definition;
		...
		if (this.forceAll || this.scale[0] !== xScale || this.scale[1] !== yScale) {
			this.scale[0] = xScale;
			this.scale[1] = yScale;
		...
		if (this.forceAll || hotspotX !== this.hotspot[0] || hotspotY !== this.hotspot[1]) {
		...
		if (this.forceAll || !Utils.equal4(this.corners, cornerA, cornerB, cornerC, cornerD)) {
		...
		if (this.forceAll || !Utils.equal3(this.pos, newPosX, newPosY, newPosZ)) {
		...
		if (this.forceAll || !Utils.equal3(this.motion.mov, newMovX, newMovY, newMovZ)) {
			Utils.set3(this.motion.mov, newMovX, newMovY, newMovZ);
			updateTimes.move = now;
		}

		const newMotionTime = time.get(instanceIndex);
		if (this.forceAll || this.motion.time !== newMotionTime) {
		...
		if (this.forceAll || !Utils.equal3(this.motion.gravity, newGravityX, newGravityY, newGravityZ)) {
		...
		if (this.forceAll || !Utils.equal3(this.rotation.angle, angleX, angleY, angleZ)) {
		...
		if (this.forceAll || !Utils.equal3(this.rotation.center, centerX, centerY, centerZ)) {
